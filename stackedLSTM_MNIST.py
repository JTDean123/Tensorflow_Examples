###################################################
#
# Build a stacked LSTM in tensorflow to predict digits from MNIST data
# Jason Dean
# 09/16/17
#
###################################################
...
def stack_lstm(mnist):

    # each image is 28X28, or 784 pixels
    # we will input a 28 sequences 28 times into the RNN
    # define placeholders
    ...
    # define a stacked lstm_cell with 25 hidden nodes in layer
    def lstm_cell():
        return tf.contrib.rnn.BasicLSTMCell(25)
    ...
    outputs, states = rnn.static_rnn(stacked_lstm, x_, dtype=tf.float32)
...
def run():
    ...
    plt.title('Loss vs. Epoch for Stacked LSTM', fontsize=20)
    plt.show()
